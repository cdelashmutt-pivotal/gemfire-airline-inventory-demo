...
if __name__ == '__main__':
    ...
    renderTemplate(templateDir,'storage.json.tpl', context, configDir)
    print('{0} rendered'.format(os.path.join(configDir, 'storage.json')))
    ...
    with open(os.path.join(configDir,'storage.json'), 'r') as cfFile:
    ...
    storageDescriptor = os.path.join(here, 'aws_runtime_storage.json')
    with open(storageDescriptor, 'w') as f:
        json.dump(volTable, f, indent = 3)

    print('EBS volume information written to "{0}"'.format(storageDescriptor))
